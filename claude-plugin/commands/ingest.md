...
- **Single-graph mode** (`raw_roam_server` == `roam_server`, or `raw_mode: "namespace"`, § Single-graph mode): write to the **one** graph as a **`RAW/<Source Title>`** page (namespace prefix from `raw_namespace`, default `RAW/`) with the same attributes. **Copy vs move:** if the body lives *outside* the graph (URL, uploaded file) **extract** it into the RAW page; if the body was *pasted into the daily note* (meeting notes, email, freeform text) **`roam_move_block`** it under the RAW page — do **not** copy, or the body ends up duplicated in both the daily note and RAW. Then leave a `Source: [[RAW/<Source Title>]] [[META/Log]]` link in the daily note where the body was. The daily note is the inbox; the user may drop URLs/docs/emails/text there or ask you to attach material into it first. The hub is ephemeral — write **nothing to disk**; that daily-note `[[META/Log]]` block is the log record (collect the log via `[[META/Log]]` backlinks; no content-log page) — see `references/roam-backend.md` § Single-graph mode → Operational state.
