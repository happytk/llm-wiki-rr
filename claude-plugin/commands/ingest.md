...
- **Single-graph mode** (`raw_roam_server` == `roam_server`, or `raw_mode: "namespace"`, § Single-graph mode): the **daily note is the canonical inbox — every capture lands on *today's* daily note first** (see `references/roam-backend.md` § Single-graph mode → "The daily note is the canonical ingest inbox"). If the user hands a source straight to ingest and it isn't on the note yet, drop a block for it onto today's daily note as you archive it. Write the captured source to the **one** graph as a **`RAW/<Source Title>`** page (namespace prefix from `raw_namespace`, default `RAW/`) with the same attributes. **Copy vs move:** if the body lives *outside* the graph (URL, uploaded file) **extract** it into the RAW page; if the body was *pasted into the daily note* (meeting notes, email, freeform text) **`roam_move_block`** it under the RAW page — do **not** copy, or the body ends up duplicated in both the daily note and RAW. The user's **own thought blocks** on the daily note are *not* archived to RAW — they stay on the note as evidence and are folded into the wiki at compile time. Then leave a `Source: [[RAW/<Source Title>]] [[META/Log]]` link in the daily note where the body was. The hub is ephemeral — write **nothing to disk**; that daily-note `[[META/Log]]` block is the log record (collect the log via `[[META/Log]]` backlinks; no content-log page) — see `references/roam-backend.md` § Single-graph mode → Operational state.
