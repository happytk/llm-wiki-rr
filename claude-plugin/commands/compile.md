...
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls:*), Bash(wc:*), Bash(date:*), Bash(mv:*), Bash(mkdir:*), mcp__roam-direct, mcp__roam, mcp__roam-wiki, mcp__roam-archive, mcp__wiki, mcp__wiki-raw, mcp__wiki-s
---
...
> **Content language:** if `content_language` is set in config (e.g. `ko`), write the synthesized prose — abstract, body, summaries, See-Also descriptions — in that language, keeping technical terms, proper nouns, code, schema/enum values, namespace prefixes, and ordinal dates canonical. See `references/compilation.md` § Content Language.
...
>
> **Two-graph mode (raw graph → wiki graph):** when `raw_roam_server` is set and differs from `roam_server` (see `references/roam-backend.md` § Two-graph mode), determine uncompiled sources by querying the **raw graph** (`mcp__<raw_roam_server>__roam_datomic_query` for `type:: source` pages lacking a `compiled::` value, or filtered by `ingested:: [[date]]` range / a day's daily note) instead of surveying disk `raw/`. Read those source pages, synthesize, and write articles to the **wiki graph** (`mcp__<roam_server>__*`). After each source is compiled, stamp it in the raw graph with `compiled:: [[today]]` and `compiled-into:: <Article Title>`. Provenance on the article is text (`source-title::`/`source-url::`), not a cross-graph `[[link]]`. Also honor "compile the source I just added to <raw graph>" — read the page the user names.
>
> **Single-graph mode (`RAW/*` → article, one graph):** when `raw_roam_server` == `roam_server` (or `raw_mode: "namespace"`; see `references/roam-backend.md` § Single-graph mode), **ground the compile in a day's daily note** — the daily note is the canonical inbox and the evidence basis (see § Single-graph mode → "The daily note is the canonical ingest inbox"). Read the target day's daily note (default today) and gather **both** its captured-source links (→ `RAW/…` pages) **and** the user's own thought blocks typed onto the note; both are first-class evidence. Resolve which raw pages the day introduced with a `roam_datomic_query` for pages whose title starts with the `raw_namespace` (`RAW/`) and that lack a `compiled::` value. Synthesize unprefixed article pages (title must never start with `RAW/` or collide with an ordinal date) from the raw sources **and** the day's thought blocks; set `compiled-from::` to `sources`, `mixed`, or `conversation` accordingly. Provenance is a **real link**: `source:: [[RAW/<title>]]` attribute + `[[RAW/<title>]]` blocks under `Sources` (backlinks give coverage — no disk paths). Stamp each raw page with `compiled:: [[today]]` and `compiled-into:: [[<Article Title>]]` via `roam_apply_page_ops` (never rewrite the immutable raw page's content). The hub is ephemeral, so write **nothing to disk**: add one `Wiki: [[<Article Title>]] [[META/Log]] — <one-line>` block to today's daily note (that block *is* the log; `[[META/Log]]`'s backlinks collect the whole log — no content-log page) and bump `last-compiled::` on `[[META/Index]]` — all in Roam (see `references/roam-backend.md` § Single-graph mode → Operational state).
>
